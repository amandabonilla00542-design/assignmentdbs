...
export default function Login() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        const check = await fetch('https://assignmentdbs-1.onrender.com/api/IB/Welcome', {
          method: 'GET',
          credentials: 'include',
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true)
    try {
      const response = await fetch('https://assignmentdbs-1.onrender.com/api/postinglogin', {
        method: 'POST',
        credentials: 'include',
